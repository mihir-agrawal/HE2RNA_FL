...
class TranscriptomeDataset:
    ...
    def load_transcriptomes(self):
        ...
        df=df.T
        df['Sample'] = df.index
        df['Sample']=df['Sample'].apply(lambda x:x[:15])
        
        df = df.merge(self.metadata[['Sample','File.ID', 'Sample.ID',
                                     'Case.ID', 'Project.ID']],
                      on='Sample', how='inner')
        print(df.shape)
...
def main():
    ...
    dataset = TranscriptomeDataset(projectname=["TCGA_BRCA"])
    dataset.load_transcriptomes()
    dataset.transcriptomes.to_csv(os.path.join(PATH_TO_TRANSCRIPTOME, 'all_transcriptomes.csv'), index=False)
