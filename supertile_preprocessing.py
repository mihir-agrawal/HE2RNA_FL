...
def cluster_dataset(dataset, n_tiles=100,
    ...
    for x, y in tqdm(dataloader):
        ...
        #                                   yinyang_t=0,
        #                                   verbosity=0)
        # Select number of clusters (same logic as before)
        num_clusters = min(n_tiles, x.shape[0])

        # Run KMeans on the x and y coordinates
        kmeans = KMeans(n_clusters=num_clusters, random_state=42, n_init=10)
        clusters = kmeans.fit_predict(x[:, 1:3].astype('float32'))

        # Extract centroids if needed
...
def main():
    ...
    rna_data = TranscriptomeDataset.from_saved_file(args.path_to_transcriptome, projectname=["TCGA_BRCA"], genes=[])
